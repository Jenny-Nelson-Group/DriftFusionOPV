...
switch fun_type
    ...
    case 'square_sweep'
        % Generate intensity array
        fun = @(coeff, t) coeff(1)*lt(mod(t,coeff(3)).*1/coeff(3),coeff(4)/100) + (coeff(2)-coeff(1)).*min(1,t/coeff(5)).*lt(mod(t,coeff(3)).*1/coeff(3),coeff(4)/100)...
           + coeff(2).*((mod(t,coeff(3)).*1/coeff(3))>=(coeff(4)/100))+(coeff(2)-coeff(1)).*max(-1,(coeff(3)*coeff(4)/100-t)/coeff(5)).*((mod(t,coeff(3)).*1/coeff(3))>=(coeff(4)/100));
end