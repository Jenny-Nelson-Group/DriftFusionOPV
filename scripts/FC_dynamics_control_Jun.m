...
Prec.params.Ex.sigma=0.01;%gaussian distribution for CT state
...
Prec.params.CT.sigma=0.01;%gaussian distribution for CT state
...
Prec.params.Vstar=0; % Coupling between S1 and CT in eV
...
% DV2=device(DP);
% 
% DV2=device.runsolJsc(DV2,1e-6);
%     toc
% % light JV
%     tic
%     DV2=device.runsolJV(DV2,1e-6,Vstart,Vend);
%     toc
% 
...
hold on 
lg=legend;
lg.Title.String='series resistance';
subplot(2,2,4)
plot(variablelist(1:kk),PL_maxint/max(PL_maxint),'*-')
hold on
...
figure (22);set(gcf,'color','w');
saveas(figure(22),['Series resistance','.fig']);
%% Plot the results for the shunt resitance
kk=0;
clear EL_maxint PL_maxint
variablelist=[100,1000,10000];
%eval("DV2=DV_1;");
for var=variablelist
    kk=kk+1;
...
saveas(figure(33),['Shunt resistance','.fig']);
...
figure(44)
subplot(2,2,4)
semilogy(mobility./variablelist(1:kk),PL_maxint/max(PL_maxint),'*-')
hold on
% plot(mobility./variablelist(1:kk),EL_maxint_5/max(EL_maxint_5),'o-')
semilogy(mobility./variablelist(1:kk),EL_maxint_2/max(EL_maxint_2),'o-')
semilogy(mobility./variablelist(1:kk),EL_maxint_1/max(EL_maxint_1),'o-')

semilogy(mobility./variablelist(1:kk),EL_maxint_05/max(EL_maxint_05),'o-')
...
figure (44);set(gcf,'color','w');
saveas(figure(44),['Mobility','.fig']);
else
 end
